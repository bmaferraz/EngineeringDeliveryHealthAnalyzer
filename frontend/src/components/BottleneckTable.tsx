...
type BottleneckTableProps = UseBottlenecksResult & { isCompletedRelease?: boolean }
...
const BottleneckTable: React.FC<BottleneckTableProps> = ({ data, loading, error, isCompletedRelease }) => {
  if (loading) {
  ...
  return (
    <div className="rounded-2xl shadow-md p-6 bg-white">
      ...
      {bottlenecks.length === 0 ? (
        isCompletedRelease ? (
          <div className="flex items-start gap-3 rounded-xl bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 px-5 py-4">
            <div className="mt-0.5 shrink-0 w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center">
              <svg className="w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-indigo-700">Retrospective Bottleneck Analyzer with AI</p>
              <p className="text-xs text-indigo-500 mt-0.5">Coming soon — AI-powered retrospective insights for completed releases.</p>
            </div>
          </div>
        ) : (
